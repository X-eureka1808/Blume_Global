...
int main() {
    ...
    // Create a matrix (distance2) excluding the first column
    int distance2[8][7];
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 7; j++) {
    ...
    // Sort each row of the matrix distance2
    for (int i = 0; i < 8; ++i) {
    ...
    for (int p = 0; p < cities; p++) {
        for (int j = 1; j < cities + 1; j++) {
            // Check if the current city in the original matrix is equal to the sorted matrix
            if (distance[row][j] == distance2[row][d]) {
                ...
                // If the city has been visited, skip to the next iteration
                if (d != 0) break;
                ...
                if (quantity_wt > Q_wt || quantity_v > Q_v) {
                    ...
                    // Mark the current position as unreachable in the route array
                    route[i] = -1;
                    i++;
                    j = 0;
                    d++;
        ...
        // If the city is not visited, update the route and row
        if (d == 0) {
            route[i] = index;
            row = index;
            i++;
